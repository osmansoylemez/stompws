...
@property (nonatomic, retain) JFRWebSocket *webSocket;
...
+ (STOMPFrame *) STOMPFrameFromData:(NSData *)data {
    ...
    {
        command = [[contents objectAtIndex:0] copy];
    }
...
@synthesize socket, webSocket, url, host, heartbeat, port;
...
- (id)initWithURL:(NSURL *)theUrl webSocketHeaders:(NSDictionary *)headers useHeartbeat:(BOOL)heart {
    if(self = [super init]) {
        self.webSocket = [[JFRWebSocket alloc] initWithURL:theUrl protocols:WSProtocols];
        if (headers) {
            for (NSString *key in headers.allKeys) {
                [self.webSocket addHeader:[headers objectForKey:key] forKey:key];
            }
        }
...
- (void)sendFrameWithCommand:(NSString *)command
    ...
    else if (self.webSocket)
    {
        ...
        STOMPFrame *frame = [[STOMPFrame alloc] initWithCommand:command headers:headers body:body];
        LogDebug(@">>> %@", frame);
        [self.webSocket writeString:[frame toString]];
    }
}
...
- (void)sendPing:(NSTimer *)timer  {
    ...
    else if (self.webSocket)
    {
        ...
        [self.webSocket writeData:[NSData dataWithBytes:"\x0A" length:1]];
        LogDebug(@">>> PING");
    }
}
